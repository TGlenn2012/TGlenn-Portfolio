      ...
      >
        ...
        {!isUser && links && links.length > 0 && (
          <div className="mt-2 pt-2 border-t border-gray-700/50">
            <p className="text-xs text-gray-400 mb-1.5">Learn more:</p>
            <div className="flex flex-wrap gap-1.5">
              {links.map((link, index) => (
                <Link
                  key={index}
                  to={link.url}
                  className="inline-block bg-blue-600/30 hover:bg-blue-600/40 text-blue-300 px-2 py-1 rounded-full text-xs transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-gray-800"
                >
                  {link.text} →
                </Link>
              ))}
